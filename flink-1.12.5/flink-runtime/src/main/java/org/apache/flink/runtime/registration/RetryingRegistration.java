...
public abstract class RetryingRegistration<
    ...
    public void startRegistration() {
        ...
        try {
            ...
            if (FencedRpcGateway.class.isAssignableFrom(targetType)) {
                 ...
                 * 完成 TaskExecutor 到 ResourceManager 的 RPC 连接
                 */
                rpcGatewayFuture =
                        (CompletableFuture<G>)
                                rpcService.connect(
            ...
            CompletableFuture<Void> rpcGatewayAcceptFuture =
                    rpcGatewayFuture.thenAcceptAsync(
                            (G rpcGateway) -> {
                                ...
                                /**
                                 * TODO: add by antony at 2022/5/2
                                 * ResourceManager的本地代理
                                 * 执行注册
                                 * 初始时间：cluster.registration.initial-timeout = 0.1s
                                 * 关于注册失败了之后的尝试的间隔时间的机制
                                 * 1、两个配置，初始时间间隔：0.1s；最大时间间隔：30s
                                 * 2、currentTimeout = initialTimeout
                                 * 3、间隔时间的增长处理：currentTimeout = Math.min(currentTimeout * 2, maxTimeout)
                                 */
                                register(
                                        rpcGateway,
                                        1,
    ...
    private void register(final G gateway, final int attempt, final long timeoutMillis) {
        ...
        try {
             ...
             * TODO: add by antony at 2022/5/2
             * 执行注册
             * 注册者向被注册者 发送 RPC 请求，提交注册对象， 被注册者完成注册处理之后，会生成一个注册响应
             */
