...
public class DefaultDispatcherResourceManagerComponentFactory
    ...
    public DispatcherResourceManagerComponent create(
        ...
        try {
             ...
             *  内部有一个Router 类和一系列好 Handlers，来处理各种操作
             *  webMontorEndpoint = DispatcherRestEndpoint
             *
             */
            webMonitorEndpoint =
             ...
             *  1、启动 内部 netty服务端
             *  2、startInternal()
             *      2-1、启动选举，选举成功后，将自己的sessionID和address：port写入到zk中
             ...
             *      1、停止 旧的resourceManager
             *      2、更新sessionID
             *      3、创建resourceManager
             *      4、如果是leader，就调用resourceManager.start() 来启动resourceManager
             *      5、启动成功后，初始化系列服务
             *      a-1、启动心跳服务
