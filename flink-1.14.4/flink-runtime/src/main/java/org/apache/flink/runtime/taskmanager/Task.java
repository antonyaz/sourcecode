...
public class Task
     ...
     * 1、封装得到一个TaskInfo对象
     * 2、创建得到一个上下文
     * 3、初始化输出组件： ResultPartiton 和 ResultSubPartition
     * 4、初始化输入组件： InputGate + InputChannel
     * 5、初始化一个线程对象： 用来执行当前这个Task
     *
     *  上述各种事情中，涉及一个重要的组件： ShuffleEnvironment 具体实现是NettyShuffleEnvironment
     *
     * 重要的事项有-6个：
     * 1、创建 Task信息的元数据对象 TaskInfo
     */
    public Task(
         ...
         *  增加了一个 ResultPartitionConsumableNotifer功能
         *
         */
     ...
     *  区别：
     *  Spark：是批处理计算引擎，Task有开始也有结束
     *  Flink：是流处理计算引擎，Task有开始，没有结束(Task会一直运行，会一直占用这个线程)
     */
    ...
    private void doRun() {
        ...
        try {
            ...
            try {
                 ...
                 *  两种最常用的类型： SourceStreamTask、OneInputStreamTask
                 *  父类： StreamTask
                 *  通过反射来实例化StreamTask实例
                 *  1、SourceStreamTask，去看SourceStreamTask中带Environment参数的构造方法
                 *  2、OneInputStreamTask 及 TwoInputStreamTask
                 *
                 */
